...
### 🌧️ 1. เรดาร์ตรวจฝนสด กทม. & ระบบตรวจสอบสถานะ Real-Time (Live Weather Radar & Traffic)
- **เรดาร์กลุ่มฝนสด กทม. (Official BMA Live Radar Portal):** เชื่อมต่อไปยังสถานีเรดาร์ตรวจฝนหลวงของกรุงเทพมหานครโดยตรง (สถานีหนองจอก และ สถานีหนองแขม โดยสำนักการระบายน้ำ กทม.) แสดงทิศทางและความเร็วลมของกลุ่มเมฆฝนแบบสด ๆ 100% ถูกต้องแม่นยำสูงสุด
- **โหมดตรวจสอบ 2 มิติ (Dual-Mode):**
  - `🟢 ตรวจสอบสถานะสด (Live Real-Time)`: นาฬิกาดิจิทัลสด Real-time เดินตลอดเวลา พร้อมปุ่มรีเฟรชข้อมูลสด และรายชื่อ 31 เส้นทางเฝ้าระวังน้ำท่วมซ้ำซาก
  - `📂 บันทึกจุดเสี่ยงน้ำท่วม 31 สาย (Archive Snapshot)`: บันทึกประวัติศาสตร์ช่วงวิกฤตฝนตกหนัก 26 ก.ย. 11:56 น. สำหรับศึกษาจุดเสี่ยง
- **ลิงก์ตรวจจราจรสดและน้ำท่วม 1-Click:** ปุ่มเปิด Google Maps Traffic Live (`&layer=t`), รายงานจุดน้ำท่วมถนน สำนักการระบายน้ำ กทม. (สนล.), และเรดาร์ตรวจฝน กทม.

### 🗺️ 2. แผนที่อินเทอร์แอคทีฟถนนจริง ซูมเข้า-ออกได้ละเอียด (Interactive Real-World Map)
- **แผนที่จริงระดับถนน (Real Basemap):** ใช้แผนที่จาก **Esri World Street Map & Esri World Imagery (Satellite)** แสดงแนวถนนจริง สี่แยก ทางด่วน คลองระบายน้ำ และสถานีรถไฟฟ้าอย่างครบถ้วน ไร้ปัญหา "Zoom Level Not Supported" ไม่มีลายน้ำ และไม่ต้องใช้ API Key
...
- **Multi Map Mode:** สามารถกดสลับมุมมองระหว่าง `[ 🗺️ ถนน ]`, `[ 🛰️ ดาวเทียม ]`, และ `[ 📐 ผังภาพรวม ]` ได้ทันที
