...
class GrandPrixMobileGame {
  ...
  setupParticleSystem() {
    ...
    // 2. High-Speed Barrier Collision Sparks
  ...
  spawnExhaustSmoke(pos, yaw, intensity = 1.0) {
    ...
    // Small initial diameter (0.12 - 0.18m, matching exhaust pipe) expanding up to 1.8m
      ...
      this.aiRacers.forEach(racer => {
        if (this.state === 'RACING') {
          ...
          if (racer.car) {
            ...
            if (racer.controller.controls.throttle > 0.55 && Math.random() < 0.25 && typeof racer.car.getExhaustWorldPositions === 'function') {
              const aiTips = racer.car.getExhaustWorldPositions();
              if (aiTips.length >= 2) {
                const tip = Math.random() < 0.5 ? aiTips[0] : aiTips[1];
                this.spawnExhaustSmoke(tip, racer.physics.yaw, 0.6);
                if (Math.random() < 0.3) {
                  this.spawnExhaustFlame(tip, racer.physics.yaw, 0.6, false);
                }
              }
            }
          }
