/**
 * ==========================================================================
 * Professional Circuit Racing - Broadcast HUD & Telemetry
 * Real-time Tachometer, Shift Lights, Dynamic Track Mini-Map Radar,
 * Live Position Leaderboard, and Lap Timing.
 * ==========================================================================
 */
...
export class RacingHUD {
  ...
  updateGauges(physics) {
    if (this.speedEl) {
      this.speedEl.textContent = `${Math.max(0, Math.abs(physics.speedKmh))}`;
    }
    ...
    // RPM Gauge Needle Rotation (-120 deg to +120 deg)
    if (this.rpmNeedleEl) {
    ...
    if (this.shiftLightsEl) {
      const normRpm = (physics.rpm - 1000) / 8000;
  ...
  updateMiniMap(playerPhysics, allCars) {
    ...
    allCars.forEach(item => {
      ...
      ctx.beginPath();
      ctx.arc(nx * w, nz * h, 4.5, 0, Math.PI * 2);
      ctx.fill();
