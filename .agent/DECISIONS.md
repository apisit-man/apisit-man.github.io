...
## ADR-008: Scoped LocalStorage Keys for Difficulty High Scores
- **Context**: High score was stored globally under `homeSortHighScore`, meaning easy mode scores (e.g. 100 with only 3 categories) overwritten or dominated hard mode scores (5 categories).
- **Decision**: Scope keys with difficulty suffixes (`homeSortHighScore_easy`, `homeSortHighScore_medium`, `homeSortHighScore_hard`) generated by `GameLogic.getHighScoreKey()`, falling back to the legacy key if uninitialized.
- **Consequences**:
  - Fair and motivating progress tracking across different difficulty tiers.
  - Full backward compatibility with existing player records.
